...
import { RazorPayCreateOrderArguments, RazorPayPaymentPayload, RazorPayPaymentVerificationArguments, RazorPayVerificationPayload } from './types';
...
export async function handleRazorPayOrderPayload(
    ...
    const options = {
        ...
        receipt: crypto.createHash('md5').update
            (payload.cartId).digest('hex').substring(0, 20),
        notes: {
...
export async function handleRazorPayPaymentVerificationPayload(
    ...
    const bodyHashed = crypto.createHmac('sha256', args.key_secret).update(`${args.orderCreationId}|${args.razorpayPaymentId}`).digest('hex');
